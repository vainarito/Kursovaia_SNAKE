...
void MainWindow::openSettings(QString& COLOR) {
    ...
    QLabel *label = new QLabel(tr("Settings"));
        ...
        // Добавьте виджет в основной макет
        layout->addWidget(volumeWidget);

    //---------------------------------------------------------------------------- настройка звука

    //---------------------------------------------------------------------------- цвета змейки
        // Создайте виджет для выбора цвета змейки
        ...
        if (index != -1) { // -1 means the item was not found
        ...
        colorMap.insert(0, 1); // Green
        colorMap.insert(1, 2); // Red
        colorMap.insert(2, 3); // Yellow
        colorMap.insert(3, 4); // Blue
        colorMap.insert(4, 5); // White
    ...
    //---------------------------------------------------------------------------- звук от кнопок
        ...
        soundWidget->setStyleSheet("background-color: #232339; border-radius: 10px;"); // Установите цвет фона и закругление углов
        ...
        //soundCheckBox->setChecked(true); // По умолчанию звук включен
    ...
    layout->addStretch(1);
    // Подключите слот изменения значения слайдера к методу setVolume() плеера
...
{
    QMessageBox::information(this, tr("Помощь"), tr("Здесь ваш текст помощи..."));
}
    ...
    : QMainWindow(parent)
{
    setWindowFlags(Qt::FramelessWindowHint);
        ...
        player = new QMediaPlayer(this);
            // Установите медиафайл, который нужно проиграть
...
void MainWindow::openImageWindow() {
    ...
    // Создайте виджет для информации об авторизованном пользователе
    QWidget *userWidget = new QWidget();
    userWidget->setStyleSheet("background-color: #232339; border-radius: 10px;");
    userWidget->setFixedHeight(150);  // Установите высоту плашки
    ...
    SoundButton *backButton = new SoundButton("Back");  // Это строка из вашего исходного кода
    ...
    :QMainWindow(parent), m_mainWindow(mainWindow)
{
    setWindowFlags(Qt::FramelessWindowHint);
        ...
        //connect(m_gameField, &GameField::altPressed, this, &MainWindow::showMainWindow); //---------------
    ...
    : QWidget(parent)
{
    ...
    layout->addStretch(); // Добавляем растягивающийся элемент, чтобы кнопки были справа
