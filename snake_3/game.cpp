...
GameField::GameField()
{
    // игровое поле и его границы
    setFixedSize(300, 300);
    setFocusPolicy(Qt::StrongFocus);

    //размер одного элемента на поле
    m_sizeItem = 10;
...
{
    QString text;
    // постоновка игры на пузу и наоборот
    if(m_isPause)
    {
        m_moveSnakeTimer->stop();
        m_bonusTimer->stop();
        text = "Count: " + QString::number(m_score) + "\nContinue - SPACE" ;
    }
    else
    {
...
void GameField::StartNewGame()
{
    ...
    m_moveSnakeTimer->start(100);
    m_score = 0;
    QString text = "Count: " + QString::number(m_score) + "\nPause - SPACE";
    emit ChangeTextSignal(text);
}

// отрисовка всего что происходит во время игрового процесса
void GameField::paintEvent(QPaintEvent *e)
{
    ...
    if(m_isGameOver)
    {
        ...
        painter.drawText(QRect(0, 0, width(), height()), Qt::AlignCenter, "THE GAME IS LOST\nCount : " + QString::number(m_score));
        return;
    }
...
QBrush snakeBrushHead(snakeColorValueHead, Qt::SolidPattern);
    ...
    {
        // Если это голова змейки, изменяем цвет
        if(i == 0)
        {
